...
class MutualInformationPredictor(nn.Module):
    """Vision Transformer"""
    ...
    def __init__(
        ...
        **kwargs
        ...
        predictor_proj = nn.Linear(predictor_embed_dim, embed_dim)
        if use_spectral_norm:
        ...
        self.predictor_head = (
            nn.Sequential(
                ...
                nn.ReLU(),
                spectral_norm(nn.Linear(int(mlp_ratio * embed_dim), 1)),
            )
            if use_spectral_norm
            else nn.Sequential(
                nn.Linear(
                ...
                nn.ReLU(),
                nn.Linear(int(mlp_ratio * embed_dim), 1),
            )
        )
    ...
    def f(self, ctx, tgt, masks_enc, masks_pred):
        ...
        tgt_pos_embed = repeat_interleave_batch(
            tgt_pos_embed, B, repeat=len(masks_enc)
        )  # [B * len(masks_pred) * len(masks_enc), N_tgt, predictor_embed_dim]

        # -- Mask token for prediction
        tgt_mask_tokens = self.mask_token.repeat(
            tgt_pos_embed.size(0), tgt_pos_embed.size(1), 1
        )
        tgt_mask_tokens += tgt_pos_embed

        # -- Concating ctx and tgt_mask_tokens
        _, N_ctxt, _ = ctx.shape
        x = torch.cat(
            [ctx, tgt_mask_tokens], dim=1
        )  # [B * len(masks_pred) * len(masks_enc), N_ctx + N_tgt, predictor_embed_dim]
        ...
        # -- Select tgt pred tokens
        tgt_pred_tokens = x[:, N_ctxt:]
        tgt_pred_tokens = self.predictor_proj(
            tgt_pred_tokens
        )  # [B * len(masks_pred) * len(masks_enc), N_tgt, embed_dim]
        ...
        out = out.mean(dim=1)  # [B * len(masks_pred) * len(masks_enc), embed_dim]
        out = self.predictor_head(out)  # [B * len(masks_pred) * len(masks_enc), 1]
        out = out.squeeze(-1)  # [B]

        return out, tgt_pred_tokens, tgt

    def forward(self, ctx, tgt, masks_enc, masks_pred):
        # Compute network outputs for joint samples
        t_joint, tgt_pred, tgt_true = self.f(ctx, tgt, masks_enc, masks_pred)
        ...
        mse = F.mse_loss(tgt_pred, tgt_true)

        # Total loss is a weighted sum of MSE and negative mutual information lower bound
        loss = (self.mse_factor * mse) - (self.mi_factor * mi_lb)

        return loss, mi_lb, mse


class MutualInformationCrossPredictor(nn.Module):
    """Vision Transformer"""
    ...
    def f(self, ctx, tgt, masks_enc, masks_pred):
        ...
        # -- Mask token for prediction
        tgt_pred_tokens = self.mask_token.repeat(
            tgt_pos_embed.size(0), tgt_pos_embed.size(1), 1
        )
        tgt_pred_tokens += tgt_pos_embed

        # -- Fwd prop
        for blk in self.predictor_blocks:
            tgt_pred_tokens = blk(ctx, tgt_pred_tokens)
        tgt_pred_tokens = self.predictor_norm(tgt_pred_tokens)
