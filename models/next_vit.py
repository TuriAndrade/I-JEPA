r""" 
    from https://github.com/wilile26811249/Next-ViT
"""
...
class MHCA(nn.Module):
    ...
    def __init__(self, channel):
        ...
        self.grouped_conv = nn.Conv2d(
            channel, channel, kernel_size=3, padding=1, groups=channel // 32
        )
...
class NCB(nn.Module):
    ...
    def __init__(self, channel, drop_path=0.0):
        super().__init__()
...
class E_MHSA(nn.Module):
    ...
    def __init__(self, dim, heads=8, inner_dim=64, dropout=0.0, stride=2):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x):
        ...
        q = (
            q.view(b, h * w, self.heads, self.inner_dim)
            .permute(0, 2, 1, 3)
            .contiguous()
        )
        ...
        attn = torch.softmax(
            attn, dim=-1
        )  # [b, heads, h * w, s_h * s_w], s_h = s_h // stride
...
class NextVit(nn.Module):
    def __init__(self, in_channels=3, stage3_repeat=2, num_class=1000, drop_path=0.0):
        super().__init__()
